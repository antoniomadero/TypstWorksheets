...
<arduino-8-led-diod-různé-efekty-pwm-stmívání>
#strong[Počet LED:] 8 ks \
#strong[Piny:] 2, 3, 4, 5, 6, 7, 8, 9 (piny 3, 5, 6, 9, 10, 11 podporují
PWM → ideální na stmívání)



=== 1. Klasické běžící světlo (jedna LED svítí)
<klasické-běžící-světlo-jedna-led-svítí>
```cpp
...
<postupné-plnění-všechny-zůstávají-svítit>
```cpp
...
<stmívání-jednotlivých-led-po-sobě>
```cpp
...
- Experimentuj s rychlostmi a jasem pro dosažení různých vizuálních
  efektů.
- Ujisti se, že máš správně zapojené rezistory, aby nedošlo k přetížení
  LED diod.
